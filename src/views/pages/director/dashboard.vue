  ...
  <div class="row h-100 justify-content-center align-items-center">
    <div class="col align-self-stretch">
      <div>
        <b-card no-body>
          <b-card-body>
            <h4><span class="border-bottom text-black-50">{{roleObject.title}}</span>
      ...
      <div class="row">
        ...
        <slot>
          <div class="col">
            <b-button :to="{name:role + '-user-list'}" class="col pt-3 pb-3" type="button"
                      variant="light">
              <i class="fas fa-users fa-3x mr-5 ml-5"/>
              <hr/>
              Пользователи
              <b-badge variant="primary">{{users}}</b-badge>
            </b-button>
          </div>
          <div class="col">
            <b-button :to="{name:role + '-stream-list'}" class="col pt-3 pb-3" type="button"
                      variant="light">
...
<script>
  ...
  import GQLSuperuser from "~gql/dashboard/director.graphql";
  ...
  export default {
    ...
    data() {
      return {
        roleObject: new Roles().getObjectByKey(this.$route.meta.role),
        users: 0,
        streams: 0,
        organization: {
          name: '',
      ...
      getData() {
        this.$apollo.query({
          ...
          if (!errors) {
            ...
            this.users = data.me.myUsers.paginatorInfo.total;
