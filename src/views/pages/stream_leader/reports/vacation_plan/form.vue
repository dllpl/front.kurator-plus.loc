    ...
    <div class="container" style="width: 800px;">
      <b-card class="">
        ...
        <b-form inline class="mb-4" @submit="onSubmit" v-if="!readonly">
          Учитель: <b class="mx-2">{{reportFio}}</b>
          класс:
          <b-select required class="mx-1" :disabled="readonly" :options="streamsSelect"
                    v-model="model.dirty.learningStreamId" size="sm"/>
          год
          <b-select required class="mx-1" :disabled="readonly"
                    :options="[{value:null,text:'не выбрано'}].concat(...years)"
                    v-model="model.dirty.year" size="sm"/>
          сезон:
          <b-select required class="mx-1"
                    :options="[{value:null,text:'не выбрано'}].concat(...seasons.getSelectOptions())"
                    v-model="model.dirty.season"
                    :disabled="model.dirty.class === null || readonly" size="sm"/>
